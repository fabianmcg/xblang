...
struct CollapseMemRefOpLLVMLowering : public LoweringPattern<CollapseMemRefOp> {
  using Base::Base;
  LogicalResult
  matchAndRewrite(Op op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final;
...
struct FuncOpLowering : public LoweringPattern<func::FuncOp> {
  using Base::Base;
  LogicalResult
  matchAndRewrite(Op op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final;
...
struct NullptrOpLLVMLowering : public LoweringPattern<NullPtrOp> {
  using Base::Base;
  LogicalResult
  matchAndRewrite(Op op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final;
...
struct ReturnOpFuncLowering : public LoweringPattern<ReturnOp> {
  using Base::Base;
  LogicalResult
  matchAndRewrite(Op op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final;
...
class XBLangToLLVM : public xblang::xb::impl::XBLangToLLVMBase<XBLangToLLVM> {
public:
  using Base::Base;
...
bool CastOpToLLVM::ptr2Int(ConversionPatternRewriter &rewriter, Op op,
                           ...
                           bool reversed) const {
  if (reversed)
    rewriter.replaceOpWithNewOp<LLVM::PtrToIntOp>(op, convertType(rhs), value);
  else
    rewriter.replaceOpWithNewOp<LLVM::IntToPtrOp>(op, convertType(lhs), value);
  return true;
}
...
bool CastOpToLLVM::ptr2ptr(ConversionPatternRewriter &rewriter, Op op,
  ...
  if (target.getPointee() == source.getPointee() &&
    ...
    rewriter.replaceOpWithNewOp<LLVM::AddrSpaceCastOp>(op, convertType(target),
                                                       value);
  }
  return true;
}
...
LogicalResult GetElementOpLLVMLowering::matchAndRewrite(
  ...
  if (isa<LLVM::LLVMStructType>(base.getType())) {
    rewriter.replaceOpWithNewOp<LLVM::ExtractValueOp>(op, base, indx.getInt());
    return success();
  }
  rewriter.replaceOpWithNewOp<LLVM::GEPOp>(
...
FuncOpLowering::matchAndRewrite(Op op, OpAdaptor adaptor,
  ...
  if (op->getAttr("par") && opts.isOffload()) {
    rewriter.eraseOp(op);
    return success();
  }
  return failure();
...
LogicalResult NullptrOpLLVMLowering::matchAndRewrite(
    Op op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const {
  rewriter.replaceOpWithNewOp<LLVM::ZeroOp>(op, convertType(op.getType()));
  return success();
}
...
LogicalResult ReturnOpFuncLowering::matchAndRewrite(
    Op op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const {
  if (op.hasOperands())
    rewriter.replaceOpWithNewOp<LLVM::ReturnOp>(op, adaptor.getInput());
  else
...
void XBLangToLLVM::runOnOperation() {
  LLVMConversionTarget target(getContext());
  LowerToLLVMOptions opts(&getContext());
  XBLangToLLVMTypeConverter typeConverter(&getContext(), opts);
  target.addLegalOp<ModuleOp>();
  RewritePatternSet patterns(&getContext());
  populateXBLangToLLVMConversionPatterns(typeConverter, patterns);
  ...
  mlir::SymbolTable symbolTable = mlir::SymbolTable(getOperation());
  ...
  patterns.add<FuncOpLowering>(typeConverter, patterns.getContext(), 10);
...
void xblang::xb::populateXBLangGPUToLLVMConversionPatterns(
    TypeConverter &typeConverter, RewritePatternSet &patterns) {
  populateXBLangToCF(typeConverter, patterns);
  patterns.add<CastOpToLLVM, CollapseMemRefOpLLVMLowering,
               GetElementOpLLVMLowering, NullptrOpLLVMLowering,
               ReturnOpGPUFuncLowering, SizeOfOpLLVMLowering>(
      typeConverter, patterns.getContext());
}

void xblang::xb::populateXBLangToLLVMConversionPatterns(
    TypeConverter &typeConverter, RewritePatternSet &patterns) {
  populateXBLangToCF(typeConverter, patterns);
  patterns
      .add<CastOpToLLVM, CollapseMemRefOpLLVMLowering, GetElementOpLLVMLowering,
           NullptrOpLLVMLowering, ReturnOpFuncLowering, SizeOfOpLLVMLowering>(
          typeConverter, patterns.getContext());
}
