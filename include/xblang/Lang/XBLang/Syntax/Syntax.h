//===- Syntax.h - Declares the XBLang syntax ---------------------*- C++-*-===//
//
// This file is licensed under the Apache License v2.0 with LLVM Exceptions.
// See https://llvm.org/LICENSE.txt for license information.
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
//
//===----------------------------------------------------------------------===//
//
// This file declares the xblang syntax.
//
//===----------------------------------------------------------------------===//

#ifndef XBLANG_LANG_XBLANG_SYNTAX_SYNTAX_H
#define XBLANG_LANG_XBLANG_SYNTAX_SYNTAX_H

#include "xblang/Lang/XBLang/Syntax/Init.h"
#include "xblang/Lang/XBLang/Syntax/Lexer.h"
#include "xblang/Lang/XBLang/XLG/XBGDecl.h"
#include "xblang/Lang/XBLang/XLG/XBGDialect.h"
#include "xblang/Lang/XBLang/XLG/XBGExpr.h"
#include "xblang/Lang/XBLang/XLG/XBGStmt.h"
#include "xblang/Lang/XBLang/XLG/XBGType.h"
#include "xblang/Syntax/ParserBase.h"
#include "xblang/XLG/Builder.h"

#include "xblang/Lang/XBLang/Syntax/XBLangParser.h.inc"

#endif // XBLANG_LANG_XBLANG_SYNTAX_SYNTAX_H
